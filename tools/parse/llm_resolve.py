...
def _build_consistency_prompt(refinements: dict, parsed_tree: dict) -> str:
    """Build prompt for the consistency pass.

    Shows all resolved method signatures grouped by module/class, combining
    already-resolved args from the parsed tree with newly resolved args from
    the LLM refinements. This gives the LLM full visibility to spot
    cross-method and cross-class naming inconsistencies.
    """
    # Build a combined view: class -> method -> [(arg_name, arg_type), ...]
    from collections import defaultdict

    class_methods: dict[str, dict[str, list[tuple[str, str]]]] = defaultdict(dict)

    # Walk the parsed tree to get all method signatures
    def _walk(node: dict, path: str = ""):
        ...
        if "methods" in node:
            for name, method in node["methods"].items():
                ...
                class_methods[path][name] = args

    _walk(parsed_tree)

    # Apply LLM refinements on top (name changes + type changes)
    for ref_path, ref_entry in refinements.items():
        parts = ref_path.rsplit(".", 1)
        if len(parts) != 2:
            continue
        class_path, method_name = parts

        if "args" not in ref_entry:
            continue

        if class_path not in class_methods or method_name not in class_methods[class_path]:
            continue

        current_args = class_methods[class_path][method_name]
        # Build a lookup by current arg name
        arg_list = list(current_args)
        for old_name, changes in ref_entry["args"].items():
            new_name = changes.get("name", old_name)
            new_type = changes.get("type")
            for i, (aname, atype) in enumerate(arg_list):
                if aname == old_name:
                    arg_list[i] = (new_name, new_type or atype)
                    break
        class_methods[class_path][method_name] = arg_list

    # Format as compact text grouped by module
    lines = []
    current_module = None
    for class_path in sorted(class_methods.keys()):
        methods = class_methods[class_path]
        # Only include classes that have at least one method touched by refinements
        has_refined = any(
            f"{class_path}.{m}" in refinements
            for m in methods
        )
        if not has_refined:
            continue

        module = class_path.split(".")[1] if "." in class_path else ""
        ...
        for method_name in sorted(methods.keys()):
            ...
            lines.append(f"    {method_name}({arg_str})")

    signature_block = "\n".join(lines)

    return f"""## Consistency Review

You are reviewing the resolved method signatures for the Ableton Live API stubs.
All arg names and types below have been resolved by a previous pass. Your job is to find
**cross-method and cross-module naming inconsistencies** and return corrections.

### What to look for:
- Same conceptual parameter using different names across related methods
  (e.g. `channel` vs `midi_channel` for the same kind of MIDI channel arg)
- Same conceptual parameter using different names across classes in the same module
  (e.g. `index` in one class but `device_index` in another for the same kind of index)
- Inconsistent pluralization between append/extend pairs
- Names that don't match the naming style (see system prompt)

### What NOT to change:
- Don't rename args that are already consistent within their class/module
- Don't change types — only names
- Don't rename args where the different name is justified by genuinely different semantics

### Signatures (class > method > args)
...
Return a JSON object with ONLY the corrections needed. Use the same format as the main
refinements — path as key, with an `args` dict mapping the CURRENT arg name to the new name:
  ...
  "corrections": {{
    "Live.MidiMap.forward_midi_cc": {{
      "args": {{
        "channel": {{
          "name": "midi_channel",
          "name_reason": "Consistent with map_midi_cc which uses midi_channel"
        }}
      }}
    }}
  }}
...
If no inconsistencies are found, return `{{"corrections": {{}}}}`.

Respond with ONLY the JSON object, no other text."""


def consistency_pass(version: str, refinements_path: str, batch_dir: str) -> str:
    """Prepare a consistency review prompt and write it to the batch dir.

    Returns the path to the consistency prompt file.
    """
    ...
    prompt = _build_consistency_prompt(refinements, tree)

    Path(batch_dir).mkdir(parents=True, exist_ok=True)
    prompt_path = join(batch_dir, "consistency_prompt.md")
    with open(prompt_path, "w") as f:
        f.write(prompt)

    # Count methods shown
    n_lines = sum(1 for line in prompt.split("\n") if line.strip().startswith("Live."))
    print(f"Wrote consistency prompt to {prompt_path}")
    print(f"  {n_lines} classes with refined methods included")
    print(f"  {len(prompt)} chars")
    print(f"\nRun this prompt with the system_prompt.md as context.")
    print(f"Save result as {join(batch_dir, 'consistency_result.json')}.")
    return prompt_path


def apply_consistency(refinements_path: str, corrections_path: str) -> int:
    """Apply consistency corrections to the refinements file.
    ...
    applied = 0
    for path, entry in corrections.items():
        if path not in refinements:
            print(f"  Warning: {path} not in refinements, skipping")
            continue
        for old_name, changes in entry.get("args", {}).items():
            ref_args = refinements[path].get("args", {})
            if old_name in ref_args:
                new_name = changes.get("name")
                if new_name:
                    ref_args[old_name]["name"] = new_name
                    ref_args[old_name]["name_reason"] = changes.get(
                        "name_reason", ref_args[old_name].get("name_reason", ""))
                    applied += 1
                    print(f"  {path}.{old_name} -> {new_name}")
            else:
                print(f"  Warning: {path}.{old_name} not in refinements args, skipping")

    with open(refinements_path, "w") as f:
        json.dump(data, f, indent=2)
    print(f"\nApplied {applied} corrections to {refinements_path}")
    return applied
...
def main():
    ...
    parser.add_argument("--consistency", action="store_true",
                        help="Prepare consistency review prompt (post-merge)")
    parser.add_argument("--apply-consistency", action="store_true",
                        help="Apply consistency corrections from consistency_result.json")
    ...
    if args.consistency:
        consistency_pass(args.version, output_path, batch_dir)
        return

    if args.apply_consistency:
        corrections_path = join(batch_dir, "consistency_result.json")
        apply_consistency(output_path, corrections_path)
        return
