...
class StubGenerator:
    ...
    def _write_init_file(
        ...
        *,
        ...
        """Write {Module}/__init__.py with helper classes, enums, and functions."""
        buf = StringIO()
        exports: list[str] = []
    ...
    def _write_top_init(self, module_names: list[str]) -> None:
        """Write Live/__init__.py that imports all submodules."""
        out_file = join(self.output_dir, "__init__.py")
        with open(out_file, "w", encoding="utf-8") as f:
    ...
    def _render_class(self, node: dict, buf: StringIO, indent: int, path: str = "") -> None:
        ...
        if node.get("constructable") and node.get("init_doc"):
            self._render_init(node, buf, indent + 1)
            has_body = True
    ...
    def _render_init(self, node: dict, buf: StringIO, indent: int) -> None:
        ...
        for i, m in enumerate(args):
            ...
            arg_type = self._resolve_probed_type(m.group(1))
            arg_name = m.group(2)
            default = m.group(3)
    ...
    def _resolve_probed_type(self, probed_type: str | None) -> str:
        ...
        *,
        ...
        for cls in sorted(referenced):
            ...
            if source_module == module_name:
                ...
                # For __init__.py, skip — name is defined locally
            else:
