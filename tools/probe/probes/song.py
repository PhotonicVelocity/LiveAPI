...
SONG_SETTABLE_PROPS: list[tuple[str, Any]] = [
    ...
    # back_to_arranger excluded — engine-driven status flag, set may be ignored without session clip trigger
...
def probe_undo(ctx: ProbeContext) -> None:
    ...
    # _probe_back_to_arranger_undo(ctx)  # need to trigger session clip
...
def _oid_set(items: list[dict[str, Any]]) -> set[str]:
    """Extract OIDs from a list of resolved children."""
    return {item["oid"] for item in items}


def _find_new_index(items: list[dict[str, Any]], before_oids: set[str]) -> int | None:
    """Find the index of the first item whose OID wasn't in before_oids."""
    for i, item in enumerate(items):
        if item["oid"] not in before_oids:
            return i
    return None


def _probe_method_undo_create_scene(ctx: ProbeContext) -> None:
    """Probe Song.create_scene undo — create a scene at the end, undo, check OIDs."""
    oids_before = _oid_set(ctx.resolve_children(ctx.song_oid, "scenes"))

    def check_fn() -> bool:
        return bool(_oid_set(ctx.resolve_children(ctx.song_oid, "scenes")) - oids_before)

    def cleanup_fn() -> None:
        scenes = ctx.resolve_children(ctx.song_oid, "scenes")
        idx = _find_new_index(scenes, oids_before)
        if idx is not None:
...
def _probe_method_undo_create_midi_track(ctx: ProbeContext) -> None:
    oids_before = _oid_set(ctx.resolve_children(ctx.song_oid, "tracks"))

    def check_fn() -> bool:
        return bool(_oid_set(ctx.resolve_children(ctx.song_oid, "tracks")) - oids_before)

    def cleanup_fn() -> None:
        tracks = ctx.resolve_children(ctx.song_oid, "tracks")
        idx = _find_new_index(tracks, oids_before)
        if idx is not None:
...
def _probe_method_undo_create_return_track(ctx: ProbeContext) -> None:
    oids_before = _oid_set(ctx.resolve_children(ctx.song_oid, "return_tracks"))

    def check_fn() -> bool:
        return bool(_oid_set(ctx.resolve_children(ctx.song_oid, "return_tracks")) - oids_before)

    def cleanup_fn() -> None:
        returns = ctx.resolve_children(ctx.song_oid, "return_tracks")
        idx = _find_new_index(returns, oids_before)
        if idx is not None:
            ctx.call(ctx.song_oid, "delete_return_track", [idx])
            ctx.release()
...
def _probe_method_undo_set_or_delete_cue(ctx: ProbeContext) -> None:
    ...
    oids_before = _oid_set(ctx.resolve_children(ctx.song_oid, "cue_points"))

    def check_fn() -> bool:
        return _oid_set(ctx.resolve_children(ctx.song_oid, "cue_points")) != oids_before
