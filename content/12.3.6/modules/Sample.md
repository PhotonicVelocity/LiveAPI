...
```yaml
kind: property
type: Live.Sample.TransientLoopMode
settable: true
listenable: true
...
refinement:
  element_type:
    confidence: high
    sources:
    - '[M4L] sample.md:68 says "list of int read-only" + "positions of all playable slices in sample frames".'
    - '[corpus] Push2/timeline_navigation.py iterates and accesses positions.'
```
...
```yaml
kind: property
type: Live.Sample.SlicingBeatDivision
settable: true
listenable: true
...
refinement:
  type:
    ...
    confidence: high
    sources:
    - '[sister method] `WarpMode` enum lives in Clip but drives both `Clip.warp_mode` and `Sample.warp_mode` (the sample inside
      an audio clip). Cross-module but unambiguous — only one `WarpMode` enum exists across the LOM.'
```
