...
- name: file_path
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] clipslot.md uses `path`. Renamed to `file_path` for clarity (avoids conflation with the dotted-path concept;
        explicitly indicates a file system path). Names are positional-only and decorative — clarity over doc-conformance.'
returns:
  type: Live.Clip.Clip
raw_doc: |-
...
- name: length
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[C++ signature] `create_clip(TPyHandle<AGroupAndClipSlotBase>, double)` — single float, semantically a length.'
      - '[docstring] "Creates an empty clip with the given length in the slot."'
      - '[M4L] clipslot.md: "Parameter: `length` — Length is given in beats and must be a greater value than 0.0."'
      - '[corpus] binding callsites all pass a variable named `length` (or `_clip_length`): _Framework/ClipCreator.py:20,
        ableton/v2/control_surface/clip_creator.py:21, ableton/v3/control_surface/components/step_sequence.py:17, novation/print_to_clip.py:133,
        APC64/render_to_clip.py:141.'
      - '[corpus] the previous `pages` rename was wrong: its single def-vote came from pushbase/loop_selector_component.py:390
        `def create_clip(pages)`, a nested local closure inside `_on_press_loop_selector_matrix` that takes a page count and
        converts it to a length before calling a different helper — no connection to the binding.'
returns:
  type: Live.Clip.Clip
raw_doc: |-
...
- name: target_clip_slot
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clipslot.md names the parameter `target_clip_slot`.'
returns:
  type: None
raw_doc: |-
...
- name: state
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clipslot.md names the parameter `state`.'
returns:
  type: None
