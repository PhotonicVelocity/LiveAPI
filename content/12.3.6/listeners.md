...
title: Listeners
slug: listeners
include_module: Listener
sidebar_badge: core
_note: |
  Foundation page describing the LOM subscription model. The
  `listen` property-flag chip on every observable property's
  reference page links here. Hoisted above the alphabetical Modules
  group in the sidebar (see `web/astro.config.mjs`). The two
  bookkeeping classes (`ListenerHandle` / `ListenerVector`) are
  appended below this prose via the `include_module:` directive.
---
