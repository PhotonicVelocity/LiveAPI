...
typed inventory of every reachable class, property, method, and enum, ready for autocomplete and static type
checking in your Control Surface or Remote Script.

**Setup.** Add the stubs directory to your type checker's stub path:

```jsonc
// pyrightconfig.json
{
  "stubPath": "stubs/12.3.6"
}
```

The package includes a `py.typed` marker for PEP 561 compatibility, so any tool that respects PEP 561 picks it up.
