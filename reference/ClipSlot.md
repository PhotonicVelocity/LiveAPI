...
- **Primary:** `Live/classes/ClipSlot.py` (stub dump)
- **Secondary:** `MaxForLive/clipslot.md`
- **Probes:** `probe_clipslot_color.py`, `probe_clipslot_color_set_group.py`, `probe_will_record.py`,
  `probe_controls_other_clips.py` (inline)

### Probe Notes

#### Color behavior (probed 2026-02-17)

- `color` and `color_index` have **no setter** — `set` raises `"property of 'ClipSlot' object has no setter"` on both
  regular and group slots.
- **Regular slots** (empty or with clip): both return `None`. Even when `has_clip=True`, the slot's own color is `None`
  — the clip's color is only accessible via `clip.color`.
- **Group slots** (empty row): both return `None`.
- **Group slots** (clips in child tracks): returns the **first** contained clip's color/color_index. In a group with
  children at `color_index=5` (green) and `color_index=22` (blue), the group slot reported `color_index=5`.
- The Live UI allows "setting" a group slot's color, but this is a UI-level composite action that sets `Clip.color` on
  each child clip individually — it is not exposed through `ClipSlot.color`.
...
- Tested scenarios (6-track session: 1-MIDI, 2 Group containing 3 MIDI + 4 Audio, 5 Group containing 6 Audio):
  - Empty row → `False`
...
- **Follow actions do not affect the value** — clips with "next" follow actions on regular
  (non-group) tracks still report `False`.
- Visually corresponds to the hatched clip indicator shown on the right side of group slots
  in the Live UI.
...
- The property reads without error but never reports `True`. May only function through the
  Max LOM path or within listener callback contexts. **Skipped** from the public API spec.
...
- ~~What does `color` / `color_index` return when `has_clip=True` on a non-group slot?~~ **Resolved:** returns `None` —
  see probe notes.
...
| ------ | ------------ | -------- | ------ | ---------- | --------------- | ------------------------------------------------ |
| `clip` | `Clip\|None` | `single` | `get`  | `yes`      | `<11`           | The clip owned by this slot, or `None` if empty. |
...
- **Shape:** `single`
- **Access:** `get`
- **Listenable:** `yes` (via `has_clip` listener — `clip` itself is not directly listenable, but `has_clip` fires when a
  clip is created or deleted)
- **Available Since:** `<11`
- **Sources:** `stub | max`
...
| ---------------------- | ---------------------- | ----------- | ---------- | --------------- | -------------------------------------------------------------------------------------- |
| `color`                | `int\|None`            | `<11`       | `yes`      | `<11`           | Color of the first clip in a group slot. `None` for regular empty slots.               |
| `color_index`          | `int\|None`            | `<11`       | `yes`      | `<11`           | Color index of the first clip in a group slot. `None` for regular empty slots.         |
| `controls_other_clips` | `bool`                 | —           | `yes`      | `<11`           | Group slot only: `True` if child tracks have active (non-deactivated) clips.           |
| `has_clip`             | `bool`                 | —           | `yes`      | `<11`           | `True` if a clip exists in this slot.                                                  |
| `has_stop_button`      | `bool`                 | `bool`      | `yes`      | `<11`           | Whether this slot has a stop button. When fired while empty, stops the track.          |
| `is_group_slot`        | `bool`                 | —           | `no`       | `<11`           | `True` if this slot belongs to a group track.                                          |
| `is_playing`           | `bool`                 | —           | `no`       | `<11`           | `True` if the clip in this slot is playing. Works on both group and non-group slots.   |
| `is_recording`         | `bool`                 | —           | `no`       | `<11`           | `True` if the clip in this slot is recording. Works on both group and non-group slots. |
| `is_triggered`         | `bool`                 | —           | `yes`      | `<11`           | `True` while a clip launch, stop, or record button in this slot is blinking.           |
| `playing_status`       | `ClipSlotPlayingState` | —           | `yes`      | `<11`           | Aggregate playback state for group slots only; always `0` for non-group slots.         |
| `will_record_on_start` | `bool`                 | —           | `no`       | `<11`           | `True` if firing this slot will begin recording. Currently always returns `False`.     |

#### `color`

- **Get Returns:** `int | None`
- **Set Accepts:** — (no setter exists; set raises error)
- **Listenable:** `yes`
- **Available Since:** `<11`
...
- **Set Accepts:** —
- **Listenable:** `yes`
- **Applicable to:** `group slots only`
- **Available Since:** `<11`
- **Sources:** `stub | max | probe`
- **Probe Status:** `probed`
...
- **Get Returns:** `ClipSlotPlayingState` (int)
- **Set Accepts:** —
- **Listenable:** `yes`
- **Available Since:** `<11`
- **Sources:** `stub | max`
...
| ------------------------------------------------------------------------ | ------- | --------------- | ----------------------------------------------------------- |
| `create_audio_clip(path: str)`                                           | `Clip`  | `11.3`          | Create an audio clip from a file in this slot.              |
| `create_clip(length: float)`                                             | `Clip`  | `<11`           | Create an empty MIDI clip of the given length in beats.     |
| `delete_clip()`                                                          | `None`  | `<11`           | Delete the clip in this slot.                               |
| `duplicate_clip_to(target_slot: ClipSlot)`                               | `None`  | `<11`           | Copy this slot's clip to another slot.                      |
| `fire(record_length=None, launch_quantization=None, force_legato=False)` | `None`  | `<11`           | Fire the clip, trigger the stop button, or begin recording. |
| `set_fire_button_state(state: bool)`                                     | `None`  | `<11`           | Programmatically hold or release the clip launch button.    |
| `stop()`                                                                 | `None`  | `<11`           | Stop the playing or recording clip on this track.           |

#### `create_audio_clip(path: str)`

- **Returns:** `Clip`
- **Args:** `path: str` — absolute path to a valid audio file
- **Raises/Errors:** Error if slot is non-empty, track is not an audio track, track is frozen, or path is not a valid
  audio file.
- **Undo-tracked:** `Unknown`
- **Side Effects:** Creates a new audio clip in this slot referencing the given file.
...
- **Raises/Errors:** Error if slot is non-empty or track is not a MIDI track.
- **Undo-tracked:** `Unknown`
- **Side Effects:** Creates a new empty MIDI clip of the specified length in this slot.
- **Async visibility:** `Unknown`
- **Available Since:** `<11`
- **Sources:** `stub | max`
- **Probe Status:** `unprobed`
...
- **Returns:** `None`
- **Args:** None
- **Raises/Errors:** Exception if the slot is empty.
- **Undo-tracked:** `Unknown`
- **Side Effects:** Removes the clip from this slot. The slot becomes empty (`has_clip=False`).
- **Async visibility:** `Unknown`
- **Available Since:** `<11`
- **Sources:** `stub | max`
- **Probe Status:** `unprobed`
...
- **Raises/Errors:** Exception if this slot is empty; if source and target track types differ (audio vs MIDI); if source
  or target is a group slot.
- **Undo-tracked:** `Unknown`
- **Side Effects:** Copies this slot's clip to `target_slot`, overriding any existing clip there.
- **Async visibility:** `Unknown`
- **Available Since:** `<11`
- **Sources:** `stub | max`
- **Probe Status:** `unprobed`
...
- **Raises/Errors:** Error if `record_length` is passed but the slot already owns a clip.
- **Undo-tracked:** `no`
- **Side Effects:** Fires the contained clip, or triggers the stop button if empty and `has_stop_button=True`, or begins
  recording if empty and the track is armed.
- **Async visibility:** `Unknown`
- **Available Since:** `<11`
- **Sources:** `stub | max`
- **Probe Status:** `unprobed`
...
- **Undo-tracked:** `no`
- **Side Effects:** Simulates holding (`True`) or releasing (`False`) the clip launch button.
- **Async visibility:** `Unknown`
- **Available Since:** `<11`
- **Sources:** `stub | max`
- **Probe Status:** `unprobed`
...
- **Raises/Errors:** `Unknown`
- **Undo-tracked:** `no`
- **Side Effects:** Stops the playing or recording clip on this track (or all child tracks for a group slot).
- **Async visibility:** `Unknown`
- **Available Since:** `<11`
- **Sources:** `stub | max`
- **Probe Status:** `unprobed`
