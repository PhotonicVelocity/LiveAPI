...
```yaml
kind: property
type: Live.SimplerDevice.PlaybackMode
settable: true
listenable: true
...
refinement:
  type:
    ...
    confidence: high
    sources:
    - '[sister method] `PlaybackMode` enum in same module with members classic / one_shot / slicing — matches Simpler''s three
      top-level playback modes. Distinct from `SlicingPlaybackMode` (mono / poly / thru), which scopes to slicing sub-mode.'
```
...
```yaml
kind: property
type: Live.SimplerDevice.SlicingPlaybackMode
settable: true
listenable: true
