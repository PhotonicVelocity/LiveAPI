...
```yaml
kind: property
type: Live.Clip.ClipLaunchQuantization
settable: true
listenable: true
...
refinement:
  type:
    ...
    confidence: high
    sources:
    - '[schema] two enums match "launch quantization": `ClipLaunchQuantization` in Clip and `Quantization` in Song. The Clip-specific
      one is the right fit — its members include `q_global` (defer to song-level) and clip-specific values; `Song.Quantization`
...
args:
- name: notes
  type: Iterable[Live.Clip.MidiNoteSpecification]
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md names the parameter `notes`.'
    type:
      probed: object
      confidence: high
      sources:
      - '[C++ signature] `boost::python::api::object` (generic) — the binding doesn''t enforce a specific vector class at
        the C++ level.'
      ...
      - '[corpus] both tuples and lists work — tuples (`(note,)` at pushbase/note_editor_component.py:611, ableton/v3/control_surface/components/note_editor.py:367)
        and lists (`note_specifications` at _MxDCore/MxDCore.py:820+).'
returns:
...
- name: warp_marker
  ...
  refinement:
    type:
      ...
      - '[corpus] runtime expects a `WarpMarker` instance — the only constructible class in `Live.Clip` matching the "warp
        marker" semantics; `Clip.warp_markers` returns a `WarpMarkerVector` of these.'
returns:
  type: None
raw_doc: |-
  Available for AudioClips only.
...
- name: notes
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[docstring] "list of notes as returned from get_notes_extended".'
returns:
  type: None
raw_doc: |-
...
- name: device_parameter
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[docstring] "return the envelope for the given parameter".'
returns:
...
- name: device_parameter
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md names the parameter `device_parameter`.'
returns:
  type: None
...
- name: device_parameter
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[docstring] "creates an envelope for a given parameter".'
returns:
  type: Live.Envelope.Envelope
...
deprecated:
  replaced_by: get_notes_extended
```
...
- name: note_ids
  ...
  refinement:
    type:
      ...
      - '[corpus] _MxDCore/MxDCore.py:864 invokes `lom_object.get_notes_by_id(id_to_note_mapping.keys())` — passes a `dict_keys`
        view (Iterable[int]).'
...
deprecated:
  replaced_by: get_selected_notes_extended
```
...
- name: beats
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md names the parameter `beats`.'
returns:
  type: None
raw_doc: |-
...
args:
- name: quantization_grid
  type: Live.Clip.GridQuantization | int
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md names arg2 `quantization_grid`.'
    type:
      probed: int
      confidence: high
      sources:
      - '[schema] enum-arg convention: the M4L name `quantization_grid` establishes the GridQuantization context.'
      - '[sister method] `Clip.View.grid_quantization` property (typed `GridQuantization`) confirms the enum context, and
        `Clip.quantize_pitch` uses the same `GridQuantization | int` annotation.'
- name: amount
  type: float
  refinement:
    name:
      probed: arg3
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md names arg3 `amount`.'
returns:
  type: None
...
- name: note
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[corpus] 6/7 callsite defs use `note` (drum_group_component.py:88, _Framework/DrumGroupComponent.py:331, etc.).'
      - '[M4L] external/max-for-live-docs/9.0/clip.md uses `pitch` — corpus def-vote dominates here, but `pitch` is the documented
        form if drift surfaces.'
- name: quantization_grid
  type: Live.Clip.GridQuantization | int
  refinement:
    name:
      probed: arg3
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md names arg3 `quantization_grid`.'
      - '[corpus] previous rename to `source` was wrong: it came from pushbase/quantization_component.py:125 `def quantize_pitch(self,
        note, source=None)`, where `source` is a UI tag string (e.g. "slice", "pad") used only in show_notification at line
        129 and NOT passed to the binding. The actual binding call at line 128 passes `self.settings.quantize_to` — a GridQuantization
        enum int.'
    type:
      probed: int
      confidence: high
...
- name: amount
  ...
  refinement:
    name:
      probed: arg4
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md names the parameter `amount`.'
      - '[sister method] `quantize(quantization_grid, amount)` confirms.'
returns:
  type: None
...
- name: from_time
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[C++ signature] pattern matches `get_notes` — float arg in the time-range slot.'
- name: from_pitch
  type: int
  refinement:
    name:
      probed: arg3
      sources:
      - '[C++ signature] pattern matches `get_notes` — int arg in the pitch-range slot.'
- name: time_span
  type: float
  refinement:
    name:
      probed: arg4
      sources:
      - '[C++ signature] pattern matches `get_notes` — float arg in the time-span slot (paired with from_time).'
- name: pitch_span
  type: int
  refinement:
    name:
      probed: arg5
      sources:
      - '[C++ signature] pattern matches `get_notes` — int arg in the pitch-span slot (paired with from_pitch).'
returns:
  type: None
raw_doc: Delete all notes starting in the given pitch- and time range.
deprecated:
  replaced_by: remove_notes_extended
```
...
- name: note_ids
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md describes it as "list of note IDs"; rendered as `note_ids` here.'
      - '[sister method] matches the arg name on `select_notes_by_id`.'
    type:
      probed: object
      confidence: high
      sources:
      - '[C++ signature] `boost::python::api::object` (generic, runtime-checked).'
      - '[corpus] iterables work via _MxDCore/MxDCore.py `_object_perform_operation_on_notes_by_id_handler`, which passes
        the `function_parameters` tuple straight through to the binding.'
returns:
  type: None
raw_doc: |-
...
- name: notes
  ...
  refinement:
    ...
    type:
      ...
      confidence: high
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md:345 confirms tuple-of-tuples format matching `get_selected_notes`: (pitch,
        time, duration, velocity, mute) per note.'
returns:
  type: None
raw_doc: |-
...
- name: note_ids
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md describes the parameter as "list of note IDs"; rendered as `note_ids`
        here.'
      - '[sister method] matches the arg name on `remove_notes_by_id`.'
    type:
      probed: object
      confidence: high
      sources:
...
- name: state
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip.md names the parameter `state`.'
returns:
  type: None
...
- name: notes
  ...
  refinement:
    ...
    type:
      ...
      sources:
      - '[docstring] "tuple of tuples where each inner tuple represents a note".'
      - '[M4L] external/max-for-live-docs/9.0/clip.md:327 confirms tuple-of-tuples format matching `get_notes`: (pitch, time,
        duration, velocity, mute) per note.'
returns:
  type: None
raw_doc: |-
  Called with a tuple of tuples where each inner tuple represents
...
- name: device_parameter
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/clip_view.md documents the type only (`Parameter: [DeviceParameter]`) without
        naming the arg. Renamed to `device_parameter` for clarity since the type is `DeviceParameter`; positional-only names
        are decorative.'
returns:
  type: None
