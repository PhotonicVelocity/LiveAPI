...
refinement:
  type:
    ...
    confidence: high
    sources:
    - '[probe] Song is the LOM root for the document tree. The probe observes None because Song has no parent (`Application.song`
      returns the Song; there''s no inverse). Same structural reasoning as `Application.canonical_parent`.'
```
...
```yaml
kind: property
type: Live.Song.SessionRecordStatus
settable: false
listenable: true
...
- name: index
  ...
  refinement:
    type:
      ...
      confidence: medium
      sources:
      - '[M4L] external/max-for-live-docs/9.0/song.md describes an optional index parameter; usage shows the function called
        with no args (default = None means "append at end").'
...
- name: index
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/song.md names the parameter `index`.'
returns:
...
- name: target
  type: Live.Track.Track | Live.Chain.Chain
  refinement:
    type:
      ...
      - '[corpus] Push2 (`device_navigation.py:247,257,264,269`) passes only `device.canonical_parent` (a Track or Chain)
        or a Chain obtained via `rack.view.selected_chain` — never any other LomObject. Track and Chain are the only LOM classes
...
- name: smpte_format
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/song.md names it `format`. Renamed to `smpte_format` for clarity (avoids shadowing
        the builtin `format()` and explicitly indicates SMPTE time-code format) — clarity over doc-conformance since the names
        are positional-only.'
returns:
...
- name: default_value
  ...
  refinement:
    type:
      ...
      confidence: high
      sources:
      - '[corpus] passes int (3, 5), bool (True), str defaults across pushbase / ableton.v2 / ableton.v3 — the binding accepts
        arbitrary persistent data, so `Any` is the honest parameterization for both arg and return.'
returns:
  type: Any
  refinement:
    type:
...
- name: beats
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/song.md names the parameter `beats`.'
returns:
  type: None
...
- name: target
  ...
  refinement:
    type:
      ...
      - '[corpus] Push2 (`device_navigation.py:247,257,264,269`) only passes Track or Chain. Narrowed to the two LOM classes
        that actually contain devices.'
- name: target_position
  type: int
returns:
  type: int
...
- name: value
  ...
  refinement:
    type:
      ...
      confidence: high
      sources:
      - '[corpus] passes int (interval), bool (is_horizontal), arbitrary persistent data — pushbase/instrument_component.py:88-110
        shows the pattern. Any matches.'
returns:
  type: None
...
- name: device
  ...
  refinement:
    name:
      probed: arg2
      sources:
      - '[M4L] external/max-for-live-docs/9.0/song_view.md names the parameter `device`.'
...
returns:
  type: tuple[tuple[str, tuple[int, ...]], ...]
  refinement:
    type:
      probed: tuple[tuple, ...]
      confidence: high
      sources:
      - '[corpus] pushbase/melodic_pattern.py:52 unpacks `Scale(name=x[0], notes=x[1]) for x in Live.Song.get_all_scales_ordered()`
